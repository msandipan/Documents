#define siameseRPM here

import torch
import torch.nn as nn
import torch.nn.functional as F

from utils.xcorr import xcorr_fast, xcorr_depthwise
...
class UPChannelRPN(RPN):
    def __init__(self, anchor_num=5, feature_in=256):
        ...
        self.template_cls_conv = nn.Conv3d(feature_in,
                feature_in * cls_output, kernel_size=3)
        self.template_loc_conv = nn.Conv3d(feature_in,
                feature_in * loc_output, kernel_size=3)

        self.search_cls_conv = nn.Conv3d(feature_in,
                feature_in, kernel_size=3)
        self.search_loc_conv = nn.Conv3d(feature_in,
                feature_in, kernel_size=3)

        self.loc_adjust = nn.Conv3d(loc_output, loc_output, kernel_size=1)
